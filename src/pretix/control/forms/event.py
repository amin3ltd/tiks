...
class EventWizardFoundationForm(forms.Form):
    locales = forms.MultipleChoiceField(
        ...
        help_text=_('Choose the languages people can use.')
    )
...
class EventWizardBasicsForm(I18nModelForm):
    ...
    no_taxes = forms.BooleanField(
        label=_("Skip tax for now"),
        help_text=_("You can add tax later."),
        required=False,
    )
    tax_rate = forms.DecimalField(
        label=_("Tax rate"),
        help_text=_("If you need tax, enter the percent. You can change this later."),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'timezone' not in self.initial:
            self.initial['timezone'] = self.organizer.settings.timezone or get_current_timezone_name()
        if 'currency' not in self.initial:
            self.initial['currency'] = settings.DEFAULT_CURRENCY
        self.fields['name'].label = _('Event name')
        self.fields['name'].help_text = _('Example: Addis Music Night')
        self.fields['slug'].label = _('Short link name')
        self.fields['slug'].help_text = _('Use small letters. Example: addis-music-night')
        self.fields['currency'].label = _('Money')
        self.fields['date_from'].label = _('Start date and time')
        self.fields['date_to'].label = _('End date and time')
        self.fields['location'].label = _('Place')
        self.fields['locale'].label = _('Main language')
        self.fields['locale'].choices = [(a, b) for a, b in settings.LANGUAGES if a in self.locales]
        self.fields['location'].widget.attrs['rows'] = '3'
        if self.organizer.settings.region == 'ET':
...
class EventSettingsValidationMixin:
    ...
    def clean(self):
        data = super().clean()
        if data.get('invoice_address_company_required') and 'invoice_address_required' in data:
            data['invoice_address_required'] = True
        if data.get('invoice_address_required') and 'invoice_address_asked' in data:
            data['invoice_address_asked'] = True
...
class EventSettingsForm(EventSettingsValidationMixin, FormPlaceholderMixin, SettingsForm):
    ...
    name_scheme = forms.ChoiceField(
        label=_("Name format"),
        help_text=_("This defines how tiks will ask for human names. Changing this after you already received "
...
class QuickSetupForm(I18nForm):
    ...
    payment_banktransfer__enabled = forms.BooleanField(
        ...
        help_text=_("Your customers will be instructed to wire the money to your account. You can then import your "
                    "bank statements to process the payments within tiks, or mark them as paid manually."),
        required=False
    )
