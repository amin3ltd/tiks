...
class UpdateSettingsForm(SettingsForm):
    update_check_perform = forms.BooleanField(
        ...
        help_text=_("During the update check, tiks will report an anonymous, unique installation ID, "
                    "the current version of tiks and your installed plugins and the number of active and "
                    "inactive events in your installation to servers operated by the tiks developers. We "
    ...
    update_check_email = forms.EmailField(
        ...
        help_text=_("We will notify you at this address if we detect that a new update is available. This "
                    "address will not be transmitted to tiks.cc, the emails will be sent by this server "
                    "locally.")
    )
